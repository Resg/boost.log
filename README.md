...
```
[Пример 62,2](#example622) основан на [примере 62.1](#example621), но в отличии от 62.1, он заменяет **boost::sources::logger** на **boost::sources::severity_logger**.
...
Тип уровня лога зависит от параметра шаблона, передаваемого в **boost::sources::severity_logger**. [Пример 62,2](#example622) использует int.
...
**boost::log::attribute_value_set**. Функции фильтра получают напосредственно **boost::log::attribute_value_set**, но не **boost::log::record**.
**Boost::log::attribute_value_set** хранит пары ключ/значение. Примерно как **std::unordered_map**.

Записи лога состоят из атрибутов. Атрибуты имеют имя и значение. Можно создавать атрибуты самостоятельно. Они также могут
создаваться автоматически – например логгерами. Именно поэтому, Boost.Log предоставляет несколько средств ведения
лога. **Boost::log::Sources::severity_logger** добавляет атрибут, называющийся уровнем точности для каждой записи лога. Этот
атрибут сохраняет уровень лога. Таким образом фильтр может проверить, является ли уровень логирования записи лога больше 0.

**Boost::log::attribute_value_set** предоставляет несколько функций-членов для доступа к атрибутам. Они похожи на 
...
созданное с помощью конструктора по умолчанию, если преобразование завершается неудачей, – например нулем в случае **int**.
...
Для безопасных типов преобразований, Boost.Log предоставляет функцию-гость  **boost::log::visit()**, которую можно использовать вместо
**extract()**.
...
Фронтэнды предоставляют функции-члены **set_formatter()**, которые могут быть переданы функции format. Если запись не фильтруется фронтэндом, она передается функции format. Эта функция форматирует запись лога как строку, которая затем передается от фронтэнда к
...
**Boost::log::record_view** предоставляет функцию-член **attribute_values()**, которая возвращает константную ссылку на
...
правой стороне макроса как **BOOST_LOG**. Полный список имен доступных атрибутов написан в документации.
...
Во всех примерах используются атрибуты определенные в Boost.Log. Пример 62,6 показывает, как создать
...
Глобальный атрибут создается путем вызова **add_global_attribute()**. Атрибут является глобальным, потому что он автоматически
...
атрибут будет являться числом записей журнала, начиная с 1.

**add_global_attribute()** не является шаблоном функции. **Boost::log::Attributes::Counter** не передается в качестве параметра шаблона. Тип
...
Атрибут Timestamp добавляется в запись лога “error”. Timestamp существует только в области, где используется
...
Функция format получает доступ к ключевым словам, чтобы написать номер строки и текущее время. Значение timestamp будет пустой строкой для
...
. Как и в случае с итераторами, верхняя граница является исключительным и не входит в диапазон.
...
Интерфейсный **boost::log::sinks::synchronous_sink** обеспечивает синхронный доступ к бекэнду, который позволяет использовать бекэнд в многопоточном приложении, даже если бекэнд не является потокобезопасным.
...
для создания строки, которая записывается в бекенд. Вместо этого строка будет именем файла, записи лога будут записаны.
...
```
Boost.Log предоставляет возможность для централизованной обработки исключений в рамках записи лога. Это означает, что вам не нужно
j,jhfxbdfnm каждый **BOOST_LOG** в блоке **try** для обработки исключений в catch.
...
**operator()** для каждого ожидаемого типа исключения. Экземпляр этого объекта функции передается **set_exception_handler()** через
...
используйте макрос **BOOST_LOG_INLINE_GLOBAL_LOGGER_DEFAULT**, как в [примере 62.10](#example6210). Передайте имя средства ведения журнала в качестве
первого параметра и тип в качестве второго. Средство ведения журнала не доступно через его имя. Вместо этого вы звоните **get()**, которая
...
глобальные средства ведения журнала. **BOOST_LOG_INLINE_GLOBAL_LOGGER_CTOR_ARGS** позволяет передавать параметры конструктора глобального логгера. Все эти макросы гарантируют, что глобальный логгер будет правильно инициализирован.

Boost.Log предоставляет много новых функций, которые стоит посмотреть. Например можно настроить инфраструктуру лога через
контейнер с парами ключ/значение в виде строк. Затем вам не нужно создавать экземпляры классов и вызова функции-члена. Например, можно
задать ключ назначения через консоль, которая автоматически создаст инфраструктуру лога используя **boost::log::sinks::text_ostream_backend**. Этот бекэнд можно настроить через дополнительный ключ/значение пары. Поскольку
